...
История релизов: [CHANGELOG.txt](CHANGELOG.txt) · Идеи и планы: [BACKLOG.txt](BACKLOG.txt) · Деплой: [deploy/README.md](deploy/README.md)

## 📋 О проекте

Интеллектуальный Telegram-помощник, который:
...
- **aiogram 3** — асинхронная Telegram Bot API
- **PostgreSQL 15** + **SQLAlchemy 2 (async)** + **Alembic** — БД, миграции
- **llama.cpp** + **Qwen3-4B (Q4_K_M)** — локальная LLM, без внешних API
- **BGE-M3** + **bge-reranker-v2-m3** — bi-encoder + cross-encoder retrieval
- **Tesseract OCR** — извлечение текста из сканированных PDF
